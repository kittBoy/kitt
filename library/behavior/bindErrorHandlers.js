...
module.exports = function bindErrorHandlers (kitt, app) {
	...
	var default404Handler = function (req, res) {
		...
		res.status(404).send(kitt.wrapHTMLError('Sorry, no page could be found at this address (404)'));
	};

	app.use(function (req, res, next) {
		var err404 = kitt.get('404');
		if (err404) {
			try {
	...
	var default500Handler = function (err, req, res, next) { // eslint-disable-line no-unused-vars
		...
		return res.status(500).send(kitt.wrapHTMLError('Sorry, an error occurred loading the page (500)', msg));
	};

	app.use(function (err, req, res, next) {
		var err500 = kitt.get('500');
		if (err500) {
			try {
