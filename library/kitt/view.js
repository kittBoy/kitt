...
function View (req, res) {
	...
	if (!req || req.constructor.name !== 'IncomingMessage') {
		throw new Error('Kitt.View Error: Express request object is required.');
	}

	if (!res || res.constructor.name !== 'ServerResponse') {
		throw new Error('Kitt.View Error: Express response object is required.');
	}
...
/**
 * 加入一个或多个方法到并行执行队列 init, action,render
 *
 * @api public
 */
...
View.prototype.on = function (on) {
	...
	if (typeof on === 'function') {
		...
		/* 第一个参数是function，则将第二个参数加入action队列
		 *
		 * Example:
		 *
		 *     view.on(function() {
 ...
 * Examples:
 *
 * view.query('books', kitt.list('Book').model.find());
 *
 ...
 * view.query(
 *     'admin.books',
 *      kitt.list('Book').model.find().where('user', 'Admin')
 * );
 *
		...
		query.exec(function (err, results) {
			...
			if (err) {
				if ('err' in callbacks) {
					 ...
					 *
					 * Example:
					 *     view.query('books', kitt.list('Book'))
				...
				if ((!results || (utils.isArray(results) && !results.length)) && 'none' in callbacks) {
					...
					if (utils.isFunction(callbacks.then)) {
						...
						return kitt.populateRelated(results, callbacks.then, next);
					}
				}
			}
...
View.prototype.render = function (renderFn, locals, callback) {
	...
	if (typeof renderFn !== 'function') {
		throw new Error('Kitt.View.render() renderFn must be a templatePath (string) or a function.');
	}
	...
	// Add Kitt's global pre('render') queue
	kitt.getMiddleware('pre:render').forEach(function (fn) {
	...
	async.eachSeries(this.initQueue, function (i, next) {
		if (Array.isArray(i)) {
			...
			throw new Error('Kitt.View.render() events must be functions.');
		}
	}, function (err) {
